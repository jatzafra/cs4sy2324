package exercise09truthzafrajulian;
...
public class Trainer {
    ...
    public void sureCapture(Monster m) throws AlreadyCapturedException, FullTeamException{
        if(team.contains(m)){
            throw new AlreadyCapturedException(m.getName() + " is already captured.");
        }
        if(team.size() == 6){
    ...
    public void release(Monster m) throws NotInTeamException{
        if(!(team.contains(m))){
            throw new NotInTeamException(m.getName() + " is not in team.");
        }
